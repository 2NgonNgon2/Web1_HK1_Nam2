...
let cartTotalAmountNumber = document.getElementById('cart-container-bottom-total-amount-number');
let numberTmp=0;
let cartContainItem = "";
...
function addProductToCartByInforProduct(id) { 
    ...
    if (product[i].id == id) {
      for (let i = temp.length-1; i >= 0; i--) {
        if(temp[i] == id) {
            ...
            for (let i = temp.length-1; i >= 0; i--) {
              if(temp[i] == id) {
                let tmpSaveValue = parseInt(cartContainerMiddle.children[i].children[2].children[0].value) + x;
                updateAddTotal(x,parseInt(cartContainerMiddle.children[i].children[3].children[0].innerText));
  ...
  for (let index = 0; index < btnRemoveProductInCarts.length; index++) {
    ...
    btnRemoveProductInCart.addEventListener("click", function(e) {
      ...
      updateSubtractTotal(parseInt(parent.children[2].children[0].value),parseInt(parent.children[3].children[0].innerText));
    ...
    if (product[i].id == id) {
      for (let i = temp.length-1; i >= 0; i--) {
        if(temp[i] == id) {
            ...
            for (let i = temp.length-1; i >= 0; i--) {
              if(temp[i] == id) {
                let tmpSaveValue = parseInt(cartContainerMiddle.children[i].children[2].children[0].value) + 1;
                updateAddTotal(1,parseInt(cartContainerMiddle.children[i].children[3].children[0].innerText));
                cartContainerMiddle.children[i].children[2].innerHTML = '';
                cartContainerMiddle.children[i].children[2].innerHTML = `
                  <input type="number" value="${tmpSaveValue}" class="cart-container-middle-product-quantity-adjust" min="1">
                `;
...
function checkEventInputValueCart() {
  for (let index = 0; index < cartContainerMiddle.children.length; index++) {
    var inputValueQuantityCart = cartContainerMiddle.children[index].children[2].children[0];
