  ...
  switch (typeProduct) {
    case "keyboard": {
      for (let i = 1; i < product.length; i++) {
        if (product[i].type === typeProduct) renderProduct(product[i]);
      }
      break;
    }
    case "headphone": {
      for (let i = 1; i < product.length; i++) {
        if (product[i].type === "headphone") renderProduct(product[i]);
      }
      break;
    }
    case "mouse": {
      for (let i = 1; i < product.length; i++) {
        if (product[i].type === "mouse") renderProduct(product[i]);
      }
      break;
    }
    case "speaker": {
      for (let i = 1; i < product.length; i++) {
        if (product[i].type === "speaker") renderProduct(product[i]);
      }
      break;
    }
    case "mousepad": {
      for (let i = 1; i < product.length; i++) {
        if (product[i].type === "mousepad") renderProduct(product[i]);
      }
      break;
    }
  }
